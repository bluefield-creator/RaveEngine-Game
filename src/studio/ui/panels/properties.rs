...
pub fn draw_properties(
    ...
    let Ok((_, mut transform, mut name, _, _, Some(_brick), mut shape_opt, _, _, mat_opt, studs_mat_opt, mut phys_opt)) = properties_query.get_mut(selected_entity) else {
        return;
    };
    ...
    if let Some(studs_mat_handle) = studs_mat_opt {
        if let Some(mat) = studs_materials.get(&studs_mat_handle.0) {
            current_color = Some(mat.base.base_color);
            is_extended = true;
        }
    } else if let Some(mat_handle) = mat_opt {
        if let Some(mat) = materials.get(&mat_handle.0) {
            current_color = Some(mat.base_color);
        }
    }

    let base_color_val = current_color.unwrap_or(Color::srgb(0.84, 0.24, 0.16));

    let mut color_array = [
        base_color_val.to_srgba().red,
        base_color_val.to_srgba().green,
        base_color_val.to_srgba().blue,
        base_color_val.to_srgba().alpha,
    ];
            ...
            .show(ui, |ui| {
                egui::Grid::new("properties_transform_grid")
                    ...
                    .show(ui, |ui| {
                        ...
                        let mut scale_val = transform.scale;
                        let mut scale_changed = false;
                        ui.horizontal(|ui| {
                            ui.label("X");
                            scale_changed |= ui.add(egui::DragValue::new(&mut scale_val.x).speed(0.1)).changed();
                            ui.label("Y");
                            scale_changed |= ui.add(egui::DragValue::new(&mut scale_val.y).speed(0.1)).changed();
                            ui.label("Z");
                            scale_changed |= ui.add(egui::DragValue::new(&mut scale_val.z).speed(0.1)).changed();
                        });
                        if scale_changed {
                            transform.scale = scale_val.max(Vec3::splat(0.01));
                        }
                        ui.end_row();
                ...
                egui::Grid::new("properties_appearance_grid")
                    ...
                    .show(ui, |ui| {
                        ...
                        if ui.color_edit_button_rgba_unmultiplied(&mut color_array).changed() {
                            ...
                            if let Ok(mut bc) = brick_colors.get_mut(selected_entity) {
                                bc.color = new_color;
                            }
                        }
                        ui.end_row();

                        if let Some(ref mut shape_comp) = shape_opt {
                            ui.label(egui::RichText::new("Shape").color(egui::Color32::from_rgb(60, 60, 60)).size(13.0));
                            let mut current_shape = shape_comp.shape;
                            egui::ComboBox::from_id_salt("brick_shape_select")
                                .selected_text(format!("{:?}", current_shape))
                                .show_ui(ui, |ui| {
                                    ui.selectable_value(&mut current_shape, crate::common::bricks::components::BrickShape::Block, "Block");
                                    ui.selectable_value(&mut current_shape, crate::common::bricks::components::BrickShape::Sphere, "Sphere");
                                });
                            if current_shape != shape_comp.shape {

...
pub fn draw_chatbox(
    ...
    if let Some(mut menu_image) = images.get_mut(&menu_handle)
        && !matches!(menu_image.sampler, bevy::image::ImageSampler::Descriptor(_))
    {
        ...
        if (format == bevy::render::render_resource::TextureFormat::Rgba8UnormSrgb
            ...
            && let Some(ref mut data) = menu_image.data
        {
    ...
    if let Some(mut chat_image) = images.get_mut(&chat_handle)
        && !matches!(chat_image.sampler, bevy::image::ImageSampler::Descriptor(_))
    {
        let format = chat_image.texture_descriptor.format;
        if (format == bevy::render::render_resource::TextureFormat::Rgba8UnormSrgb
            || format == bevy::render::render_resource::TextureFormat::Rgba8Unorm)
            && let Some(ref mut data) = chat_image.data
        {
            for chunk in data.chunks_exact_mut(4) {
                let a = chunk[3] as f32 / 255.0;
                chunk[0] = (chunk[0] as f32 * a) as u8;
        ...
        .show(ctx, |ui| {
            ...
            ui.vertical(|ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.painter()
                        .rect_filled(rect, 4.0 * scale_factor, visual_bg_color);
                    let icon_rect = egui::Rect::from_center_size(
                        rect.center(),
                        egui::vec2(35.0 * scale_factor, 35.0 * scale_factor),
                    );
                    ui.painter().image(
                        menu_tex,
                        icon_rect,
